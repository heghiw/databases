...
LANGUAGE plpgsql
SECURITY DEFINER
-- Fixed search_path is required when exposing SECURITY DEFINER procedures.
...
BEGIN
    -- Membership duration is defined once in membership_type and reused here.
...
CREATE SEQUENCE IF NOT EXISTS bike_id_seq START WITH 1;

CREATE OR REPLACE PROCEDURE create_bicycle_sp(
    p_bike_type bike_type,
    p_bike_id text DEFAULT NULL,
...
AS $$
BEGIN
    -- Generate a bike id only when the caller does not provide one.
    IF p_bike_id IS NULL OR TRIM(p_bike_id) = '' THEN
        p_bike_id := format('BIKE-%s', nextval('bike_id_seq'));
    END IF;

    IF p_bike_id !~ '^[A-Za-z0-9_-]+$' THEN
        RAISE EXCEPTION 'Bike ID must contain only letters, digits, underscore or hyphen: %', p_bike_id;
    END IF;
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM rider_account WHERE rider_id = p_rider_id) THEN
        RAISE EXCEPTION 'Rider not found: %', p_rider_id;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM bike WHERE bike_id = p_bike_id) THEN
        RAISE EXCEPTION 'Bike not found: %', p_bike_id;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM station WHERE station_id = p_start_station_id) THEN
        RAISE EXCEPTION 'Start station not found: %', p_start_station_id;
    END IF;
    IF EXISTS (SELECT 1 FROM trip WHERE bike_id = p_bike_id AND ended_at IS NULL) THEN
        RAISE EXCEPTION 'Bike is already on an active trip: %', p_bike_id;
    END IF;
...
BEGIN
    ...
    END IF;
    IF NOT EXISTS (SELECT 1 FROM station WHERE station_id = p_end_station_id) THEN
        RAISE EXCEPTION 'End station not found: %', p_end_station_id;
    END IF;
    ...
    IF NOT FOUND THEN
        RAISE EXCEPTION 'Trip not updated: %', p_trip_id;
    END IF;
...
DROP TRIGGER IF EXISTS trip_insert_audit_trg ON trip;
CREATE TRIGGER trip_insert_audit_trg
AFTER INSERT ON trip
FOR EACH ROW
EXECUTE FUNCTION trip_insert_audit_fn();

-- Row-level trigger: Station status update after trip end
CREATE OR REPLACE FUNCTION station_status_update_fn()
RETURNS TRIGGER AS $$
BEGIN
    -- Update the destination station snapshot when a trip is closed.
    IF NEW.end_station_id IS NOT NULL AND OLD.end_station_id IS DISTINCT FROM NEW.end_station_id THEN
...
DROP TRIGGER IF EXISTS station_status_update_trg ON trip;
