-- IS-309 Assignment 1 - Bike share database schema 
begin;

drop table if exists trip cascade;
drop table if exists membership cascade;
drop table if exists rider_account cascade;
...
drop type if exists membership_kind cascade;
drop type if exists bike_state cascade;
drop type if exists bike_type cascade;

drop domain if exists longitude cascade;
drop domain if exists latitude cascade;
drop domain if exists phone_text cascade;
drop domain if exists email_text cascade;
drop domain if exists country_code2 cascade;
...
create table bike_status (
  ...
  constraint bike_status_dock_station_consistency check (
    -- A dock reference is only meaningful when the bike is assigned to a station.
    dock_id is null
...
create table station_status (
  ...
  constraint station_status_totals_match check (
    -- Store the aggregate explicitly, but keep it consistent with the type-specific counts.
