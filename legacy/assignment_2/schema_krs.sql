-- Kristiansand Bike Share Database Schema
-- PostgreSQL

begin;

-- Domains and types
create domain country_code2 as text check (value ~ '^[A-Z]{2}$');
create domain email_text as text check (position('@' in value) > 1);
create domain phone_text as text check (value ~ '^[0-9+() .-]{7,}$');
...
create table bike_status (
  ...
  constraint bike_status_dock_station_consistency check (
    dock_id is null or station_id is not null
  )
);
...
create table station_status (
  ...
  constraint station_status_totals_match check (
    bikes_available_total = bikes_available_electric + bikes_available_smart + bikes_available_classic + bikes_available_cargo
  )
);
